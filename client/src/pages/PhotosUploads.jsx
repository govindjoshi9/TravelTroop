...
export default function PhotosUploads({ addedPhotos, onChange }) {
  ...
  function makeStarPhoto(ev,filename) {
    ev.preventDefault();
      ...
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {addedPhotos.length > 0 && addedPhotos.map(link => (
          <div className="h-32 flex relative" key={link}>
            ...
            <button onClick={ev => makeStarPhoto(ev,link)} className='cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3'>
            {link === addedPhotos[0] &&(
